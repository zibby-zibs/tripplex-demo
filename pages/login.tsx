...
const Login = (props: Props) => {
	...
	const onSubmit: SubmitHandler<Inputs> = async ({ email, password }) => {
		if (login) {
			await signIn?.(email, password).then(() => setPending(false));
		}
	};
		...
		<main className="flex h-screen flex-col items-center justify-center space-y-3">
			...
			<section className="drop-shadow-md">
				<form
					...
					className=" space-y-8 rounded bg-black/75 px-6 py-10 md:mt-0 md:max-w-md md:px-14"
				>
					...
					>
						{pending ? <CircleLoader color="#36d7b7" size={25} /> : "sign In"}
					</button>
